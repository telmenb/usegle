...
<div class="flex flex-col items-center">
	{#each keys as row, i}
		<div class="flex {i === 0 ? 'ml-auto pr-8' : ''}">
			{#if i === keys.length - 1}
				<button
					class="m-0.5 h-14 w-8 select-none border border-gray-300 text-center text-xl"
			...
			{#if i === keys.length - 1}
				<button
					class="m-0.5 h-14 w-16 select-none border border-gray-300 text-center text-sm"
